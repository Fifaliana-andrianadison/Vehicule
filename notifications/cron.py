...
class SendOilChangeReminders(CronJobBase):
    ...
    code = 'notifications.send_oil_change_reminders'
    ...
    def do(self):
        ...
        for record in records:
            ...
            if not user.email or not user.profile.notification_email:
                continue
...
class SendInspectionReminders(CronJobBase):
    RUN_EVERY_MINS = 1440

    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'notifications.send_inspection_reminders'
    ...
    def do(self):
        ...
        for inspection in inspections:
            ...
            if Reminder.objects.filter(
                ...
                due_date=inspection.expiry_date,
                is_sent=True
            ).exists():
                continue
...
class SendInsuranceReminders(CronJobBase):
    RUN_EVERY_MINS = 1440

    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'notifications.send_insurance_reminders'
...
class SendRegistrationReminders(CronJobBase):
    RUN_EVERY_MINS = 1440

    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'notifications.send_registration_reminders'
    ...
    def do(self):
        ...
        for carte in cartes:
            ...
            if Reminder.objects.filter(
                vehicle=carte.vehicle,
                reminder_type='registration',
                due_date=carte.expiry_date,
                is_sent=True
            ).exists():
                continue
